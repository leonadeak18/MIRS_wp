...
data <- read.csv("data/spectra_samples.csv")
...
calibrant <- read.csv("data/spectra_calibrants.csv")
...
```{r}
#import the data
cf <- read.csv('data/spectra_diff_extract_methods.csv', header = T, sep = ',')
...
library(HDANOVA)
# Gliadins
asca_cf_gli <- cf_list$gli
asca_cf_gli_df <- data.frame(
  wavenumber = I(scale(asca_cf_gli[,-c(1,2, 118, 119, 120)])),
  method = as.factor(c(rep("Dupont", 6), rep("Schalk", 6)))
)
str(asca_cf_gli_df)

asca_model_cf_gli <- asca(wavenumber ~ method, data = asca_cf_gli_df, permute = T)

summary(asca_model_cf_gli)
scoreplot(asca_model_cf_gli, factor = 'method', legendpos = 'bottomleft')

#Glutenins
asca_cf_glu <- cf_list$glu
asca_cf_glu_df <- data.frame(
  wavenumber = I(scale(asca_cf_glu[,-c(1,2, 118, 119, 120)])),
  method = as.factor(c(rep("Dupont", 6), rep("Schalk", 6)))
)
str(asca_cf_glu_df)

asca_model_cf_glu <- asca(wavenumber ~ method, data = asca_cf_glu_df, permute = T)

summary(asca_model_cf_glu)
scoreplot(asca_model_cf_glu, factor = 'method', legendpos = 'bottomleft')
```
...
```{r}
#import the data
loq <- read.csv("data/spectra_blanks.csv")
...
### NIRS with Total Proteins

```{r}
##import the nirs data
nirs <- read.csv('data/nirs.csv')
avg_samples_drop <- do.call(rbind, avg_samples)

#combine with the total protein data
nirs <- cbind(nirs, 
              total_protein = rowSums(rm[,c(2:5)]), 
              area = avg_samples_drop$location)
str(nirs)

#modelling
nirs_model <- lm(NIRS ~ total_protein, data = nirs)

#extract the Extract coefficients
intercept <- round(coef(nirs_model)[1], 3)
slope <- round(coef(nirs_model)[2], 3)
r2 <- round(summary(nirs_model)$r.squared, 3)
pval <- signif(summary(nirs_model)$coefficients[2, 4], 3)
...
nirs_plot <- ggscatter(nirs, 
    ...
    labs(y = "Crude Protein by NIRS (g/100 g)", 
         x = "Total Proteins by ATR-MIR (g/100 g)", color = NULL) +
    scale_color_manual(values = c("#fde725", "#35b779", "#31688e", "#440154")) +
    annotate("text", x = 13, y = 9, label = r2_label, size = 6, hjust = 0) +
    annotate("text", x = 13, y = 8.5, label = eq_label, size = 6, hjust = 0) +
    annotate("text", x = 13, y = 8, label = p_label, size = 6, hjust = 0) +
...
Model ASCA would be X = Xsites + Xprotein + Xsites:protein + E
...
loadingplot(asca_model_d2, factor = 'area', label = "names")

scoreplot(asca_model_d2, factor = 'area:proteins', legendpos = 'bottomleft')
```
...
Model ASCA would be X = Xsites + E

```{r}
##import the data and remove the column that contain zero value
asca_proteins2 <- all_sp[, colSums(all_sp == 0) == 0]
asca_proteins2 <- as.matrix(scale(asca_proteins2))

##create the ASCA dataframe
asca_df2 <- data.frame(
  wavenumber = I(asca_proteins2),
  area = avg_samples$Albumins$location
)

#ASCA
library(HDANOVA)
asca_model2 <- asca(wavenumber ~ area, data = asca_df2, permute = T)

summary(asca_model2)
as.numeric(attr(scores(asca_model2, factor = "area"), "explvar")["Comp 1"])
projections(asca_model2, factor = 'area')
scoreplot(asca_model2, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model2, factor = 'area', label = "names")
```
...
```{r}
##import the data and remove the column that contain zero value
asca_proteins2_d1 <- all_sp_d1[, colSums(all_sp_d1 == 0) == 0]
asca_proteins2_d1 <- as.matrix(scale(asca_proteins2_d1))

##create the ASCA dataframe
asca_df2_d1 <- data.frame(
  wavenumber = I(asca_proteins2_d1),
  area = avg_samples$Albumins$location
)

#ASCA
library(HDANOVA)
asca_model2_d1 <- asca(wavenumber ~ area, data = asca_df2_d1, permute = T)

summary(asca_model2_d1)
scoreplot(asca_model2_d1, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model2_d1, factor = 'area', label = "names")
```
...
```{r}
##import the data and remove the column that contain zero value
asca_proteins2_d2 <- all_sp_d2[, colSums(all_sp_d2 == 0) == 0]
asca_proteins2_d2 <- as.matrix(scale(asca_proteins2_d2))

##create the ASCA dataframe
asca_df2_d2 <- data.frame(
  wavenumber = I(asca_proteins2_d2),
  area = avg_samples$Albumins$location
)

#ASCA
library(HDANOVA)
asca_model2_d2 <- asca(wavenumber ~ area, data = asca_df2_d2, permute = T)

summary(asca_model2_d2)
scoreplot(asca_model2_d2, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model2_d2, factor = 'area', label = "names")
```
...
Model ASCA would be X = Xsites + Xvariety + Xarea:variety + E

```{r}
#Import Design of Experiment
DoE2 <- read.csv('data/DoE2.csv')
DoE2[] <- lapply(DoE2, as.factor)
DoE2_expanded <- DoE2[rep(1:nrow(DoE2), each = 3), ]

##import the data and remove the column that contain zero value
asca_proteins3 <- combine(prot_data_list)
asca_proteins3 <- as.matrix(scale(asca_proteins3))

##create the ASCA dataframe
asca_df3 <- data.frame(
  wavenumber = I(asca_proteins3),
  area = DoE2_expanded$area,
  variety = DoE2_expanded$variety
)

#ASCA
library(HDANOVA)
asca_model3 <- asca(wavenumber ~ area + variety + area:variety, data = asca_df3, permute = T)

summary(asca_model3)
scoreplot(asca_model3, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model3, factor = 'area', label = "names")

scoreplot(asca_model3, factor = 'variety', legendpos = 'bottomleft')
loadingplot(asca_model3, factor = 'variety', label = "names")
```
...
```{r}
# First derivative in all spectra
spectra_d1 <- lapply(seq_along(prot_data_list), function(i){
  derivative(prot_data_list[[i]], width = 11, porder = 3, dorder = 1)
})

##import the data
asca_proteins3_d1 <- combine(spectra_d1)
asca_proteins3_d1 <- as.matrix(scale(data.frame(lapply(asca_proteins3_d1, as.numeric))))

str(asca_proteins3_d1)

##create the ASCA dataframe
asca_df3_d1 <- data.frame(
  wavenumber = I(asca_proteins3_d1),
  area = DoE2_expanded$area,
  variety = DoE2_expanded$variety
)
...
#ASCA
library(HDANOVA)
asca_model3_d1 <- asca(wavenumber ~ area + variety + area:variety, data = asca_df3_d1, permute = T)

summary(asca_model3_d1)
scoreplot(asca_model3_d1, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model3_d1, factor = 'area', label = "names")
```
...
```{r}
# Second derivative in all spectra
spectra_d2 <- lapply(seq_along(prot_data_list), function(i){
  derivative(prot_data_list[[i]], width = 11, porder = 3, dorder = 2)
})

##import the data
asca_proteins3_d2 <- combine(spectra_d2)
asca_proteins3_d2 <- as.matrix(scale(data.frame(lapply(asca_proteins3_d2, as.numeric))))

str(asca_proteins3_d2)

##create the ASCA dataframe
asca_df3_d2 <- data.frame(
  wavenumber = I(asca_proteins3_d2),
  area = DoE2_expanded$area,
  variety = DoE2_expanded$variety
)
...
#ASCA
library(HDANOVA)
asca_model3_d2 <- asca(wavenumber ~ area + variety + area:variety, data = asca_df3_d2, permute = T)

summary(asca_model3_d2)
scoreplot(asca_model3_d2, factor = 'variety', legendpos = 'bottomleft')
loadingplot(asca_model3_d2, factor = 'variety', label = "names")
```

### ASCA Analysis 4

Model ASCA would be X = Xsites + Xvariety + Xsites:variety + E with second derivative data

#### Albumins

```{r}
#import the data
asca_albumins <- prot_data_list[[1]]
asca_albumins <- derivative(asca_albumins, width = 11, porder = 3, dorder = 2)
asca_albumins <- as.matrix(scale(data.frame(lapply(asca_albumins[,-1], as.numeric))))
colnames(asca_albumins) <- sub("^X", "", colnames(asca_albumins))

#create the ASCA dataframe
asca_alb_df <- data.frame(
  wavenumber = I(asca_albumins),
  area = DoE2_expanded$area,
  variety = DoE2_expanded$variety
)
...
#ASCA
library(HDANOVA)
asca_model_alb <- asca(wavenumber ~ area + variety + area:variety, data = asca_alb_df, permute = T)

summary(asca_model_alb)
scoreplot(asca_model_alb, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model_alb, factor = 'area', label = "names")
```
...
```{r}
#import the data
asca_globulins <- prot_data_list[[2]]
asca_globulins <- derivative(asca_globulins, width = 11, porder = 3, dorder = 2)
asca_globulins <- as.matrix(scale(data.frame(lapply(asca_globulins[,-1], as.numeric))))
colnames(asca_globulins) <- sub("^X", "", colnames(asca_globulins))

#create the ASCA dataframe
asca_glo_df <- data.frame(
  wavenumber = I(asca_globulins),
  area = DoE2_expanded$area,
  variety = DoE2_expanded$variety
)
...
#ASCA
library(HDANOVA)
asca_model_glo <- asca(wavenumber ~ area + variety + area:variety, data = asca_glo_df, permute = T)

summary(asca_model_glo)
scoreplot(asca_model_glo, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model_glo, factor = 'area', label = "names")
```
...
```{r}
#import the data
asca_gliadins <- prot_data_list[[3]]
asca_gliadins <- derivative(asca_gliadins, width = 11, porder = 3, dorder = 2)
asca_gliadins <- as.matrix(scale(data.frame(lapply(asca_gliadins[,-1], as.numeric))))
colnames(asca_gliadins) <- sub("^X", "", colnames(asca_gliadins))

#create the ASCA dataframe
asca_gli_df <- data.frame(
  wavenumber = I(asca_gliadins),
  area = DoE2_expanded$area,
  variety = DoE2_expanded$variety
)
...
#ASCA
library(HDANOVA)
asca_model_gli <- asca(wavenumber ~ area + variety + area:variety, data = asca_gli_df, permute = T)

summary(asca_model_gli)
scoreplot(asca_model_gli, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model_gli, factor = 'area', label = "names")
```
...
```{r}
#import the data
asca_glutenins <- prot_data_list[[4]]
asca_glutenins <- derivative(asca_glutenins, width = 11, porder = 3, dorder = 2)
asca_glutenins <- as.matrix(scale(data.frame(lapply(asca_glutenins[,-1], as.numeric))))
colnames(asca_glutenins) <- sub("^X", "", colnames(asca_glutenins))

#create the ASCA dataframe
asca_glu_df <- data.frame(
  wavenumber = I(asca_glutenins),
  area = DoE2_expanded$area,
  variety = DoE2_expanded$variety
)
...
#ASCA
library(HDANOVA)
asca_model_glu <- asca(wavenumber ~ area + variety + area:variety, data = asca_glu_df, permute = T)

summary(asca_model_glu)
scoreplot(asca_model_glu, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model_glu, factor = 'area', label = "names")
```
...
str_alb <- read.csv("data/secondary_str_albumins.csv")
...
```{r}
## import the dataframe
str_glo <- read.csv("data/secondary_str_globulins.csv")
...
```{r}
## import the dataframe
str_gli <- read.csv("data/secondary_str_gliadins.csv")
...
```{r}
## import the dataframe
str_glu <- read.csv("data/secondary_str_glutenins.csv")
...
source('code/plotting.R')
#model2
asca_model2_plot <- asca_plot(asca_model2, ftr = 'area',NULL)
asca_model2_loading <- asca_loading(asca_model2, ftr = 'area',NULL)

#model2_d1
asca_model2_d1_plot <- asca_plot(asca_model2_d1, ftr = 'area',NULL)
asca_model2_d1_loading <- asca_loading(asca_model2_d1, ftr = 'area',NULL)
asca_model2_d1_loading

#model2_d2
asca_model2_d2_plot <- asca_plot(asca_model2_d2, ftr = 'area',NULL)
asca_model2_d2_loading <- asca_loading(asca_model2_d2, ftr = 'area',NULL)
asca_model2_d2_loading
```
...
### Area
##Raw Data
asca_model3_plot <- asca_plot(asca_model3, ftr = 'area', "Raw Data with Sampling Sites Effect")

asca_model3_loading <- asca_loading(asca_model3, ftr = 'area', "Raw Data with Sampling Sites Effect")


##First Derivative
asca_model3_d1_plot <- asca_plot(asca_model3_d1, ftr = 'area', "First Derivative with Sampling Sites Effect")
asca_model3_d1_loading <- asca_loading(asca_model3_d1, ftr = 'area', "First Derivative with Sampling Sites Effect")

##Second Derivative
asca_model3_d2_plot <- asca_plot(asca_model3_d2, ftr = 'area', "Second Derivative with Sampling Sites Effect")
library(ggplot2)

asca_model3_d2_loading <- asca_loading(asca_model3_d2, ftr = 'area', "Second Derivative with Sampling Sites Effect")


## variety
source('code/plotting.R')
##Raw Data
asca_model3_var_plot <- asca_plot2(asca_model3, ftr = 'variety', "Raw Data with Variety Effect")
asca_model3_var_loading <- asca_loading(asca_model3, ftr = 'variety', "Raw Data with Variety Effect")

## First Derivative
asca_model3_var_d1_plot <- asca_plot2(asca_model3_d1, ftr = 'variety', "First Derivative with Variety Effect")
asca_model3_var_d1_loading <- asca_loading(asca_model3_d1, ftr = 'variety', "First Derivative with Variety Effect")

## Second Derivative
asca_model3_var_d2_plot <- asca_plot2(asca_model3_d2, ftr = 'variety', "First Derivative with Variety Effect")
asca_model3_var_d2_loading <- asca_loading(asca_model3_d2, ftr = 'variety', "First Derivative with Variety Effect")
```
...
#### Albumins ####
## Area
asca_model_alb_plot <- asca_plot(asca_model_alb, ftr = 'area', "Second Derivative of Albumins with Sampling Sites Effect")
asca_model_alb_loading <- asca_loading2(asca_model_alb, ftr = 'area', "Second Derivative of Albumins with Sampling Sites Effect")

## Variety
asca_model_alb_var_plot <- asca_plot2(asca_model_alb, ftr = 'variety', "Second Derivative of Albumins with Variety Effect")
asca_model_alb_var_loading <- asca_loading2(asca_model_alb, ftr = 'variety', "Second Derivative of Albumins with Variety Effect")


#### Globulins ####
## Area
asca_model_glo_plot <- asca_plot(asca_model_glo, ftr = 'area', "Second Derivative of Globulins with Sampling Sites Effect")
asca_model_glo_loading <- asca_loading2(asca_model_glo, ftr = 'area', "Second Derivative of Globulins with Sampling Sites Effect")

## Variety
asca_model_glo_var_plot <- asca_plot2(asca_model_glo, ftr = 'variety', "Second Derivative of Globulins with Variety Effect")
asca_model_glo_var_loading <- asca_loading2(asca_model_glo, ftr = 'variety', "Second Derivative of Globulins with Variety Effect")

#### Gliadins ####
## Area
asca_model_gli_plot <- asca_plot(asca_model_gli, ftr = 'area', "Second Derivative of Gliadins with Sampling Sites Effect")
asca_model_gli_loading <- asca_loading2(asca_model_gli, ftr = 'area', "Second Derivative of Gliadins with Sampling Sites Effect")

## Variety
asca_model_gli_var_plot <- asca_plot2(asca_model_gli, ftr = 'variety', "Second Derivative of Gliadins with Variety Effect")
asca_model_gli_var_loading <- asca_loading2(asca_model_gli, ftr = 'variety', "Second Derivative of Gliadins with Variety Effect")

#### Glutenins ####
## Area
asca_model_glu_plot <- asca_plot(asca_model_glu, ftr = 'area', "Second Derivative of Glutenins with Sampling Sites Effect")
asca_model_glu_loading <- asca_loading2(asca_model_glu, ftr = 'area', "Second Derivative of Glutenins with Sampling Sites Effect")

## Variety
asca_model_glu_var_plot <- asca_plot2(asca_model_glu, ftr = 'variety', "Second Derivative of Glutenins with Variety Effect")
asca_model_glu_var_loading <- asca_loading2(asca_model_glu, ftr = 'variety', "Second Derivative of Glutenins with Variety Effect")
```

#### 3D Plotting for variety
...
###ASCA analysis 3###
##Raw data
asca_model3_var_3dplot <- asca_plot3d(asca_model3, ftr = 'variety', "Raw Data with Variety Effect")
htmlwidgets::saveWidget(asca_model3_var_3dplot, "Combine_Raw.html")

##First Derivative
asca_model3_var_d1_3dplot <- asca_plot3d(asca_model3_d1, ftr = 'variety', "First Derivative with Variety Effect")
htmlwidgets::saveWidget(asca_model3_var_d1_3dplot, "Combine_d1.html")

##Second Derivative
asca_model3_var_d2_3dplot <- asca_plot3d(asca_model3_d2, ftr = 'variety', "Second Derivative with Variety Effect")
htmlwidgets::saveWidget(asca_model3_var_d2_3dplot, "Combine_d2.html")

###ASCA analysis 4###
#albumins
asca_model_alb_var_3dplot <- asca_plot3d(asca_model_alb, ftr = 'variety', "Second Derivative of Albumins with Variety Effect")
htmlwidgets::saveWidget(asca_model_alb_var_3dplot, "alb.html")

#globulins
asca_model_glo_var_3dplot <- asca_plot3d(asca_model_glo, ftr = 'variety', "Second Derivative of Globulins with Variety Effect")
htmlwidgets::saveWidget(asca_model_glo_var_3dplot, "glo.html")

#gliadins
asca_model_gli_var_3dplot <- asca_plot3d(asca_model_gli, ftr = 'variety', "Second Derivative of Gliadins with Variety Effect")
htmlwidgets::saveWidget(asca_model_gli_var_3dplot, "gli.html")

#glutenins
asca_model_glu_var_3dplot <- asca_plot3d(asca_model_glu, ftr = 'variety', "Second Derivative of Glutenins with Variety Effect")
htmlwidgets::saveWidget(asca_model_glu_var_3dplot, "glu.html")
```
...
```{r}
f3_plot <- asca_model3_d2_plot + labs(title = NULL)
f3_loading <- asca_model3_d2_loading + labs(title = NULL)
...
```{r}
#Export ASCA score plot for albumins
fs6_alb_plot <- asca_model_alb_var_plot + labs(title = NULL) + theme(legend.position = "right")
fs6_alb_loading <- asca_model_alb_var_loading + labs(title = NULL)

figs6_alb <- ggarrange(
  # Plots to arrange
  fs6_alb_plot, fs6_alb_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  widths = c(0.5, 0.5)
)
...
#Export ASCA score plot for globulins
fs6_glo_plot <- asca_model_glo_var_plot + labs(title = NULL) + theme(legend.position = "right")
fs6_glo_loading <- asca_model_glo_var_loading + labs(title = NULL)

figs6_glo <- ggarrange(
  # Plots to arrange
  fs6_glo_plot, fs6_glo_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  widths = c(0.5, 0.5)
)
...
#Export ASCA score plot for gliadins
fs6_gli_plot <- asca_model_gli_var_plot + labs(title = NULL) + theme(legend.position = "right")
fs6_gli_loading <- asca_model_gli_var_loading + labs(title = NULL)

figs6_gli <- ggarrange(
  # Plots to arrange
  fs6_gli_plot, fs6_gli_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  widths = c(0.5, 0.5)
)
...
#Export ASCA score plot for glutenins
fs6_glu_plot <- asca_model_glu_var_plot + labs(title = NULL) + theme(legend.position = "right")
fs6_glu_loading <- asca_model_glu_var_loading + labs(title = NULL)

figs6_glu <- ggarrange(
  # Plots to arrange
  fs6_glu_plot, fs6_glu_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  widths = c(0.5, 0.5)
)
...
```{r}
#Export ASCA score plot for albumins
fs7_alb_plot <- asca_model_alb_plot + labs(title = NULL)
fs7_alb_loading <- asca_model_alb_loading + labs(title = NULL)

figs7_alb <- ggarrange(
  # Plots to arrange
  fs7_alb_plot, fs7_alb_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  legend = "top",
  widths = c(0.35, 0.65)
...
#Export ASCA score plot for globulins
fs7_glo_plot <- asca_model_glo_plot + labs(title = NULL)
fs7_glo_loading <- asca_model_glo_loading + labs(title = NULL)

figs7_glo <- ggarrange(
  # Plots to arrange
  fs7_glo_plot, fs7_glo_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  legend = "top",
  widths = c(0.35, 0.65)
...
#Export ASCA score plot for gliadins
fs7_gli_plot <- asca_model_gli_plot + labs(title = NULL)
fs7_gli_loading <- asca_model_gli_loading + labs(title = NULL)

figs7_gli <- ggarrange(
  # Plots to arrange
  fs7_gli_plot, fs7_gli_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  legend = "top",
  widths = c(0.35, 0.65)
...
#Export ASCA score plot for glutenins
fs7_glu_plot <- asca_model_glu_plot + labs(title = NULL)
fs7_glu_loading <- asca_model_glu_loading + labs(title = NULL)

figs7_glu <- ggarrange(
  # Plots to arrange
  fs7_glu_plot, fs7_glu_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  legend = "top",
  widths = c(0.35, 0.65)
