...
Model ASCA would be Y = Xsites + Xprotein + Xinteraction + E
...
Model ASCA would be Y = Xsites + E
...
Model ASCA would be Y = Xsites + Xvariety + E
...
##import the data and remove the column that contain zero value
asca_proteins3 <- all_sp[, colSums(all_sp == 0) == 0]
...
asca_df3 <- data.frame(
  ...
  area = DoE2$area,
  variety = DoE2$variety
)

#ASCA
library(HDANOVA)
asca_model3 <- asca(wavenumber ~ area + variety, data = asca_df3, permute = T)
...
#ASCA
library(HDANOVA)
asca_model3_d1 <- asca(wavenumber ~ area + variety, data = asca_df3_d1, permute = T)
...
#ASCA
library(HDANOVA)
asca_model3_d2 <- asca(wavenumber ~ area + variety, data = asca_df3_d2, permute = T)

summary(asca_model3_d2)
scoreplot(asca_model3_d2, factor = 'area', legendpos = 'bottomleft')
loadingplot(asca_model3_d2, factor = 'area', label = "names")
```
...
fig3_asca <- ggarrange(
  # Plots to arrange
  asca_model2_d2_plot, asca_model2_d2_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  legend = "top",
  widths = c(0.35, 0.65)
...
```{r}
#Export the combined PCA
figs4_asca_d1 <- ggarrange(
  # Plots to arrange
  asca_model2_d1_plot, asca_model2_d1_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  legend = "top",
  widths = c(0.35, 0.65)
...
#Export the combined PCA
figs4_asca_raw <- ggarrange(
  # Plots to arrange
  asca_model2_plot, asca_model2_loading,         
  ncol = 2, nrow = 1,       
  labels = c("A", "B"),  
  legend = "top",
  widths = c(0.35, 0.65)
