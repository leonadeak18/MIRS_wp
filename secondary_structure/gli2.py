...
# Load the FTIR data
data = pd.read_csv("data/gli2.csv", header=0)
...
gli2 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, gli2["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k"]
for i in range(5):
    plt.plot(
        x,
        gli2["amplitudes"][i] * voigt_profile(x - gli2["centers"][i], gli2["sigmas"][i], gli2["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {gli2["r_squared_adj"]:.4f}, Reduced Chi² = {gli2["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(5):
    peak_area = (
        np.trapezoid(gli2["amplitudes"][i] * voigt_profile(sorted(x) - gli2["centers"][i], gli2["sigmas"][i], gli2["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak {gli2["centers"][i]:.2f}: {peak_area}")