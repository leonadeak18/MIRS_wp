...
# Load the FTIR data
data = pd.read_csv("data/lysozyme3.csv", header=0)
...
initial_params = [
    ...
    0.0001, 1675, 5, 5, # Peak 2
    0.0002, 1656, 2, 2, # Peak 3
    0.00003, 1647.5, 1, 1, # Peak 4
    0.00001, 1642, 1, 1, # Peak 5
    0.00001, 1633, 1, 1, # Peak 6
    0.0001, 1615, 1, 1, # Peak 7
]
...
lower_bounds = [
    ...
    0, initial_params[17] - 1, 0, 0,  # Peak5
...
upper_bounds = [
    ...
    np.inf, initial_params[5] + 1, 5, 5,  # Peak2
    np.inf, initial_params[9] + 2, 7, 7,  # Peak3
    np.inf, initial_params[13] + 0, 10, 10,  # Peak4
    np.inf, initial_params[17] + 2, 10, 10,  # Peak5
    np.inf, initial_params[21] + 1, 10, 10,  # Peak6
    np.inf, initial_params[25] + 2, 10, 10,  # Peak7
]
lys3 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, lys3["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k", "orange", "brown", "purple", "pink", "gray"]
for i in range(len(lys3["centers"])):
    plt.plot(
        x,
        lys3["amplitudes"][i] * voigt_profile(x - lys3["centers"][i], lys3["sigmas"][i], lys3["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {lys3["r_squared_adj"]:.4f}, Reduced Chi² = {lys3["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(len(lys3["centers"])):
    peak_area = (
        np.trapezoid(lys3["amplitudes"][i] * voigt_profile(sorted(x) - lys3["centers"][i], lys3["sigmas"][i], lys3["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak {lys3["centers"][i]:.2f}: {peak_area}")
    
    
#taking the data for visualisation
for i in range(5):
    lys3[f"area_{i}"] = lys3["amplitudes"][i] * voigt_profile(sorted(x) - lys3["centers"][i], lys3["sigmas"][i], lys3["gammas"][i])
