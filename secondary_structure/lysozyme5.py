...
# Load the FTIR data
data = pd.read_csv("data/lysozyme5.csv", header=0)
...
lys5 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, lys5["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k", "orange", "brown", "purple", "pink", "gray"]
for i in range(len(lys5["centers"])):
    plt.plot(
        x,
        lys5["amplitudes"][i] * voigt_profile(x - lys5["centers"][i], lys5["sigmas"][i], lys5["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {lys5["r_squared_adj"]:.4f}, Reduced Chi² = {lys5["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(len(lys5["centers"])):
    peak_area = (
        np.trapezoid(lys5["amplitudes"][i] * voigt_profile(sorted(x) - lys5["centers"][i], lys5["sigmas"][i], lys5["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak {lys5["centers"][i]:.2f}: {peak_area}")
    
    
#taking the data for visualisation
for i in range(5):
    lys5[f"area_{i}"] = lys5["amplitudes"][i] * voigt_profile(sorted(x) - lys5["centers"][i], lys5["sigmas"][i], lys5["gammas"][i])
