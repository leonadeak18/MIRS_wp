...
# Load the FTIR data
data = pd.read_csv("data/glu1.csv", header=0)
...
initial_params = [
    ...
    0.0002, 1675, 5, 2, # Peak 1
    0.0003, 1667, 5, 2, # Peak 2
    0.0002, 1656, 3, 1, # Peak 3
    0.0002, 1648, 2, 5, # Peak 4
    0.00002, 1638, 1, 1, # Peak 5
    0.0001, 1628, 1, 1, # Peak 6
    0.00006, 1613, 10, 3, # Peak 7
    0.0001, 1691, 1, 1, # Peak 8
]
...
upper_bounds = [
    ...
    np.inf, initial_params[29] + 2, 2, 2,  # Peak8
]
glu1 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, glu1["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k", "r", "b"]
for i in range(7):
    plt.plot(
        x,
        glu1["amplitudes"][i] * voigt_profile(x - glu1["centers"][i], glu1["sigmas"][i], glu1["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {glu1["r_squared_adj"]:.4f}, Reduced Chi² = {glu1["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(8):
    peak_area = (
        np.trapezoid(glu1["amplitudes"][i] * voigt_profile(sorted(x) - glu1["centers"][i], glu1["sigmas"][i], glu1["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak {glu1["centers"][i]:.2f}: {peak_area}")