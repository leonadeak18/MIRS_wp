...
# Load the FTIR data
data = pd.read_csv("data/gli3.csv", header=0)
...
gli3 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, gli3["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k"]
for i in range(5):
    plt.plot(
        x,
        gli3["amplitudes"][i] * voigt_profile(x - gli3["centers"][i], gli3["sigmas"][i], gli3["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {gli3["r_squared_adj"]:.4f}, Reduced Chi² = {gli3["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(5):
    peak_area = (
        np.trapezoid(gli3["amplitudes"][i] * voigt_profile(sorted(x) - gli3["centers"][i], gli3["sigmas"][i], gli3["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak {gli3["centers"][i]:.2f}: {peak_area}")
    
#taking the data for visualisation
for i in range(5):
    gli3[f"area_{i}"] = gli3["amplitudes"][i] * voigt_profile(sorted(x) - gli3["centers"][i], gli3["sigmas"][i], gli3["gammas"][i])
