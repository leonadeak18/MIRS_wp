...
# Load the FTIR data
data = pd.read_csv("data/glo3.csv", header=0)
...
initial_params = [
    ...
    0.00006, 1690, 4, 0.001, # Peak 1
    0.0001, 1680, 7, 7, # Peak 2
    0.00025, 1655, 20, 6, # Peak 3
    0.0001, 1634, 10, 5, # Peak 4
...
glo3 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, glo3["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k"]
for i in range(5):
    plt.plot(
        x,
        glo3["amplitudes"][i] * voigt_profile(x - glo3["centers"][i], glo3["sigmas"][i], glo3["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {glo3["r_squared_adj"]:.4f}, Reduced Chi² = {glo3["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(5):
    ...
    print(f"Area under Peak {glo3["centers"][i]:.2f}: {peak_area}")
    
#taking the data for visualisation
for i in range(5):
    glo3[f"area_{i}"] = glo3["amplitudes"][i] * voigt_profile(sorted(x) - glo3["centers"][i], glo3["sigmas"][i], glo3["gammas"][i])
glo3["wavenumber"] = xy["wavenumber"]
glo3["spectra"] = xy["baseline_corrected"]    