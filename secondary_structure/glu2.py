...
# Load the FTIR data
data = pd.read_csv("data/glu2.csv", header=0)
...
initial_params = [
    ...
    0.0001, 1675, 3, 5, # Peak 2
    0.00006, 1667, 3, 5, # Peak 3
    0.00005, 1656, 3, 1, # Peak 4
    0.0002, 1648, 5, 5, # Peak 5
    0.00002, 1638, 1, 1, # Peak 6
    0.0001, 1628, 1, 1, # Peak 7
...
lower_bounds = [
    ...
    0, initial_params[9] - 1, 0, 0,  # Peak3
...
upper_bounds = [
    #Amplitude, Center, Sigma, Gamma
    np.inf, initial_params[1] + 2, 20, 20,  # Peak1
    np.inf, initial_params[5] + 1, 20, 20,  # Peak2
    np.inf, initial_params[9] + 1, 20, 20,  # Peak3
    np.inf, initial_params[13] + 2, 20, 20,  # Peak4
    np.inf, initial_params[17] + 2, 20, 20,  # Peak5
    np.inf, initial_params[21] + 2, 2, 2,  # Peak6
    np.inf, initial_params[25] + 2, 20, 20,  # Peak7
    np.inf, initial_params[29] + 2, 20, 20,  # Peak8
]
glu2 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, glu2["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = [ "g", "m", "c", "y", "k", "r", "b", "orange"]
for i in range(8):
    plt.plot(
        x,
        glu2["amplitudes"][i] * voigt_profile(x - glu2["centers"][i], glu2["sigmas"][i], glu2["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {glu2["r_squared_adj"]:.4f}, Reduced Chi² = {glu2["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(8):
    peak_area = (
        np.trapezoid(glu2["amplitudes"][i] * voigt_profile(sorted(x) - glu2["centers"][i], glu2["sigmas"][i], glu2["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak {glu2["centers"][i]:.2f}: {peak_area}")