...
# Load the FTIR data
data = pd.read_csv("data/alb4.csv", header=0)
...
initial_params = [
    ...
    0.0001, 1633, 7, 0.1, # Peak 3
    0.00005, 1623, 7, 0.1, # Peak 4
...
alb4 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, alb4["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k"]
for i in range(5):
    plt.plot(
        x,
        alb4["amplitudes"][i] * voigt_profile(x - alb4["centers"][i], alb4["sigmas"][i], alb4["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {alb4["r_squared_adj"]:.4f}, Reduced Chi² = {alb4["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(5):
    peak_area = (
        np.trapz(alb4["amplitudes"][i] * voigt_profile(sorted(x) - alb4["centers"][i], alb4["sigmas"][i], alb4["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak at {alb4["centers"][i]:.2f}: {peak_area}")