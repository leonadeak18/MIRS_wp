...
initial_params = [
    ...
    0.0002, 1675, 5, 2, # Peak 2
    0.0003, 1667, 5, 2, # Peak 3
    0.0002, 1656, 3, 1, # Peak 4
    0.0002, 1648, 5, 3, # Peak 5
    #0.00002, 1638, 1, 1,
...
upper_bounds = [
    ...
    np.inf, initial_params[25] + 2, 2, 2,  # Peak7
    #np.inf, initial_params[29] + 2, 2, 2,  # Peak8
]
glu4 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, glu4["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = [ "g", "m", "c", "y", "k", "r", "b"]
for i in range(7):
    plt.plot(
        x,
        glu4["amplitudes"][i] * voigt_profile(x - glu4["centers"][i], glu4["sigmas"][i], glu4["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {glu4["r_squared_adj"]:.4f}, Reduced Chi² = {glu4["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(7):
    ...
    print(f"Area under Peak {glu4["centers"][i]:.2f}: {peak_area}")