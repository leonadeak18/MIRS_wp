...
# Load the FTIR data
data = pd.read_csv("data/alb3.csv", header=0)
...
initial_params = [
    ...
    0.0001, 1679, 13, 0.001, # Peak 1
    0.00020, 1656, 11.5, 7, # Peak 2
    0.0001, 1637, 7, 0.1, # Peak 3
    0.00005, 1627, 7, 0.1, # Peak 4
...
alb3 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, alb3["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k"]
for i in range(5):
    plt.plot(
        x,
        alb3["amplitudes"][i] * voigt_profile(x - alb3["centers"][i], alb3["sigmas"][i], alb3["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {alb3["r_squared_adj"]:.4f}, Reduced Chi² = {alb3["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(5):
    peak_area = (
        np.trapezoid(alb3["amplitudes"][i] * voigt_profile(sorted(x) - alb3["centers"][i], alb3["sigmas"][i], alb3["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak at {alb3["centers"][i]:.2f}: {peak_area}")