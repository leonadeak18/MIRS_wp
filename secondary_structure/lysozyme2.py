...
# Load the FTIR data
data = pd.read_csv("data/lysozyme2.csv", header=0)
...
lys2 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, lys2["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k", "orange", "brown", "purple", "pink", "gray"]
for i in range(len(lys2["centers"])):
    plt.plot(
        x,
        lys2["amplitudes"][i] * voigt_profile(x - lys2["centers"][i], lys2["sigmas"][i], lys2["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {lys2["r_squared_adj"]:.4f}, Reduced Chi² = {lys2["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(len(lys2["centers"])):
    peak_area = (
        np.trapezoid(lys2["amplitudes"][i] * voigt_profile(sorted(x) - lys2["centers"][i], lys2["sigmas"][i], lys2["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak {lys2["centers"][i]:.2f}: {peak_area}")
    
    
#taking the data for visualisation
for i in range(5):
    lys2[f"area_{i}"] = lys2["amplitudes"][i] * voigt_profile(sorted(x) - lys2["centers"][i], lys2["sigmas"][i], lys2["gammas"][i])
