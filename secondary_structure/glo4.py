...
# Load the FTIR data
data = pd.read_csv("data/glo4.csv", header=0)
...
initial_params = [
    ...
    0.00006, 1689, 4, 0.001, # Peak 1
    0.00007, 1679, 7, 7, # Peak 2
    0.0002, 1655, 15, 6, # Peak 3
...
glo4 = ss.deconvolution(x, y, initial_params, lower_bounds, upper_bounds)

# Plot Results
plt.figure(figsize=(7, 5))
plt.plot(x, y, "bo", markersize=3, label="Experimental Data")
plt.plot(x, glo4["y_fit"], "r-", linewidth=2, label="Total Fit")
colors = ["g", "m", "c", "y", "k"]
for i in range(5):
    plt.plot(
        x,
        glo4["amplitudes"][i] * voigt_profile(x - glo4["centers"][i], glo4["sigmas"][i], glo4["gammas"][i]),
        colors[i],
        linestyle="--",
        label=f"Peak {i + 1}",
    )
...
plt.ylabel("Absorbance (a.u.)")
plt.legend()
plt.title(f"adjusted R² = {glo4["r_squared_adj"]:.4f}, Reduced Chi² = {glo4["chi2_red"]:.1e}")
plt.gca().invert_xaxis()
plt.show()
...
for i in range(5):
    peak_area = (
        np.trapezoid(glo4["amplitudes"][i] * voigt_profile(sorted(x) - glo4["centers"][i], glo4["sigmas"][i], glo4["gammas"][i]), sorted(x))
        / area* 100
    )
    peak_areas.append(peak_area)
    print(f"Area under Peak {glo4["centers"][i]:.2f}: {peak_area}")